...
import { PlusCircle, MoreHorizontal, Trash2, MessageSquarePlus, Bot as BotIcon } from 'lucide-react';
...
export function BotList() {
  ...
  const { selectedBot, setSelectedBot } = useBotStore();
  ...
  const handleConfirmDelete = () => {
    if (editingBot) {
      deleteMutation.mutate(editingBot.id);
    }
  };
  ...
  return (
    <>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <CardTitle className="flex items-center gap-2">
                      <div 
                        className="h-6 w-6 rounded-full flex items-center justify-center text-white font-semibold text-xs"
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingBot ? 'Edit Bot' : 'Create a New Bot'}</DialogTitle>
            <DialogDescription>
              {editingBot ? 'Update the details for your bot.' : 'Fill out the form to create a new AI assistant.'}
            </DialogDescription>
          </DialogHeader>
          <BotForm bot={editingBot} onSuccess={handleFormSuccess} />
        </DialogContent>
      </Dialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="font-semibold"> {editingBot?.name}</span>.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
